...
function AccordionItem({
  className,
  ...props
}: React.ComponentProps<typeof AccordionPrimitive.Item> & { disableBackground?: boolean }) {
  return (
    <AccordionPrimitive.Item
      data-slot="accordion-item"
      className={cn("not-last:border-b", !props.disableBackground ? 'data-open:bg-muted/50 ' : '', className)}
      {...props}
    />
  )
}
...
function AccordionContent({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "pt-0 pb-4 [&_a]:hover:text-foreground h-(--radix-accordion-content-height) [&_a]:underline [&_a]:underline-offset-3 [&_p:not(:last-child)]:mb-4 h-fit",
          className
        )}
      >
        {children}
